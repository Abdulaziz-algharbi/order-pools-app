...
**Email verification:** `AppUser.isVerified` is `false` for self-registered accounts until they redeem the emailed link at the public `/verify-email` page (`pages/auth/VerifyEmailPage.tsx`, which POSTs the token once and strips it from the URL). While unverified, `EmailVerificationBanner` (in `AppShell`, with a resend button via `useResendVerification`) is shown on every panel, and actions the backend refuses (joining a pool, requesting supplier access, creating an offer) are disabled with an inline `EmailVerificationRequired` note. Any new action the backend gates with `requireVerifiedEmail` should get the same treatment.
